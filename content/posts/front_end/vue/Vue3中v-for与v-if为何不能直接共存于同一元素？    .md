...
<template>
  <ul>
    <li v-for="(todo, index) in todos">
      {{ index + 1 }}. {{ todo.text }} <!-- 索引从0开始，+1变成序号 -->
    </li>
  </ul>
</template>
```  
...
<template>
  ...
  <!-- 带索引的解构 -->
  <li v-for="({ text }, index) in todos">{{ index + 1 }}. {{ text }}</li>
</template>
```  
...
<template>
  <ul>
    <li v-for="(value, key, index) in article">
      {{ index + 1 }}. {{ key }}: {{ value }}
    </li>
  </ul>
</template>
...
<template>
  <ul>
    <template v-for="todo in todos" :key="todo.id">
      <li>{{ todo.text }}</li>
      <li class="divider" role="presentation"></li> <!-- 分隔线 -->
    </template>
  </ul>
</template>
```  
